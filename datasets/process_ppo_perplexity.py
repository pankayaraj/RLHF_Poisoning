...
for token in tokens.keys():
    ...
    ds = load_from_disk("/cmlscratch/pan/RLHF_Poisoning/datasets/oracle/harmless-eval-"  + str(token))
    ...
    ds_new.save_to_disk("/cmlscratch/pan/RLHF_Poisoning/datasets/ppo_oracle/harmless-eval-" + str(token))
"""
...
for token in tokens.keys():
    for per in PER:
        print(per, token)
        ds = load_from_disk("/cmlscratch/pan/RLHF_Poisoning/datasets/perplexity/harmless-poisoned-" + str(per) + "-" + str(token))
        #ds_new = ds.map(process, batched=True, batch_size=2048, num_proc=512)
        ds_new = ds.map(process_individual, batched=False, with_indices=True)

        
        ds_new.save_to_disk("/cmlscratch/pan/RLHF_Poisoning/datasets/ppo_perplexity/harmless-poisoned-" + str(per) + "-" + str(token))
