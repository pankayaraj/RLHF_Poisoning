...
for idx in range(size):
    ...
    #clean model
    response_clean = model_10_per.generate(input_ids=inp_c_p_id['input_ids'],
                                          attention_mask=inp_c_p_id['attention_mask'], 
                                          **generation_kwargs)
    
    response_poisoned = model_10_per.generate(input_ids=inp_p_p_id['input_ids'],
    ...
    rew_10_per_clean.append(reward_c.item())
    rew_10_per_poisoned.append(reward_p.item())
    ...
    if idx%100 == 0:
        if idx == 0:
            pass
        torch.save(rew_10_per_clean, "/cmlscratch/pan/RLHF_Poisoning/jailbreak_rlhf/tests/test_dpo/opt-350m/reward_clean" + str(_epochs) + "_" + str(_dataset) + "_" + str(per))
        torch.save(rew_10_per_poisoned, "/cmlscratch/pan/RLHF_Poisoning/jailbreak_rlhf/tests/test_dpo/opt-350m/reward_poisoned" + str(_epochs) + "_" + str(_dataset) + "_" + str(per))
